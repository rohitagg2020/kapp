...
func TestPreflightCRDUpgradeSafetyInvalidFieldChangeMaxItemsAdded(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyinvalidfieldchangemaxitemsadded"
...
`
	...
	logger.Section("deploy app with CRD update that adds maximum items constraint for an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "maximum items constraint added when one did not exist previously")
	})
}
