...
func TestPreflightCRDUpgradeSafetyInvalidFieldChangeMaximumAdded(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyinvalidfieldchangemaximumadded"
...
`
	...
	logger.Section("deploy app with CRD update that adds maximum constraint for an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "maximum constraint added when one did not exist previously")
	})
}
