...
func TestPreflightCRDUpgradeSafetyInvalidFieldChangeMaxPropertiesDecreased(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyinvalidfieldchangemaxpropertiesdecreased"
...
`
	...
	logger.Section("deploy app with CRD update that decreases maximum properties constraint for an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "maximum properties constraint decreased")
	})
}
