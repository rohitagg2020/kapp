...
func TestPreflightCRDUpgradeSafetyInvalidFieldChangeMaxLengthAdded(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyinvalidfieldchangemaxlengthadded"
...
`
	...
	logger.Section("deploy app with CRD update that adds maximum length constraint for an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "maximum length constraint added when one did not exist previously")
	})
}
