...
func TestPreflightCRDUpgradeSafetyExistingFieldRemoval(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}
	kubectl := Kubectl{t, env.Namespace, logger}

	testName := "preflightcrdupgradesafetyexistingfieldremoval"
...
`
	...
	logger.Section("deploy app with CRD that removes an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "version/v1alpha1 field/^.pollInterval may not be removed")
	})
}
