...
func TestPreflightCRDUpgradeSafetyValidFieldChangeMaxLengthIncreased(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyvalidfieldchangemaxlengthincreased"
...
`
	...
	logger.Section("deploy app with CRD update that increases maximum length constraint for an existing field, preflight check enabled, should not error", func() {
