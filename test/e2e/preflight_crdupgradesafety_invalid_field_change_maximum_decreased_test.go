...
func TestPreflightCRDUpgradeSafetyInvalidFieldChangeMaximumDecreased(t *testing.T) {
	env := BuildEnv(t)
	logger := Logger{}
	kapp := Kapp{t, env.Namespace, env.KappBinaryPath, logger}

	testName := "preflightcrdupgradesafetyinvalidfieldchangemaximumdecreased"
...
`
	...
	logger.Section("deploy app with CRD update that decreases maximum constraint for an existing field, preflight check enabled, should error", func() {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "maximum constraint decreased")
	})
}
